...
const EditMusic = () => {
    ...
    const fetchRecordDetails = async () => {
        try {
            ...
            toast.error("Ошибка при загрузке данных релиза: " + error.response.data.error, {
                position: "bottom-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
    ...
    const addParticipant = (index) => {
        ...
        if (selectedParticipant && !newTracks[index].authors.some((mem) => mem.id === selectedParticipant)) {
            const participant = members.find((member) => member.id == selectedParticipant);
    ...
    const getAvailableMembers = (trackIndex) => {
        ...
        return members.filter((member) => !selectedMembers.some((selectedPerson) => selectedPerson.id === member.id));
    };
            ...
            tracks.forEach((track, index) => {
                if (track.file) {
                    if (track.file.type === "audio/mpeg" || track.file.type === "audio/wav") {
                        formData.append(`trackFiles`, track.file, `trackFiles_${index}.mp3`);
                    } else {
    ...
    return (
        <>
            <div className="body flex-grow-1 p-3">
                <CButton className="mb-3" onClick={() => navigate("/admin/music")} color="primary">
                    Назад
                </CButton>
                <CForm className="row g-3 needs-validation" validated={validated} onSubmit={handleSubmit}>
                    ...
                    <CCol md={8}>
                        <CFormInput type="file" feedbackInvalid="Invalid file type." id="cover" label="Обложка" onChange={handleCoverChange} />
                        {coverPreview && (
                            <CImage className="mt-3" src={coverPreview.replace("..", "")} alt="Cover Preview" style={{ maxWidth: "200px" }} />
                        )}
                    </CCol>
                    <CCol md={8}>
                        <CFormInput
                        ...
                        <div key={trackIndex} className="row g-3">
                            <CCol md={8}>
                                <CFormInput
                                    ...
                                    onChange={(e) => handleTrackChange(trackIndex, "track_name", e.target.value)}
                                    required
                                />
                            </CCol>
                            <CCol md={8}>
                                ...
                                />
                                {track.path_to_file && !track.audioPreview && (
                                    <audio className="mt-3" controls src={track.path_to_file.replace("..", "")}>
                                        Your browser does not support the audio element.
                                    </audio>
                                )}
                                {track.audioPreview && (
                                    <audio className="mt-3" controls src={track.audioPreview.replace("..", "")}>
                                        Your browser does not support the audio element.
                                    </audio>
                                )}
                            </CCol>
                            ...
                            {track.authors && track.authors.length > 0 && (
                                <CCol md={8}>
                                    ...
                                    <CListGroup>
                                        {track.authors.map((member, participantIndex) => (
